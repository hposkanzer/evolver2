...
def doAmazing(odict, args):
    ...
    for exp_dir in exp_dirs:
        ...
        try:
            exp.newGeneration(local_only=odict.has_key("local-only"))
        except Generation.NotEnoughVotes:
            logger.debug("Not enough votes.  Skipping evolution of %s..." % (exp))
        except:
            msg = apply(traceback.format_exception, sys.exc_info())
            logger.warning("Error evolving %s:\n%s" % (exp, msg))
